            ...
            return this.each(function() {
               ...
               // elem_html += '</div>';
                ...
                if(settings.customTools){
                	$.each(settings.customTools,function(i,d){
                		tools_html += '<div class="btn-group  pull-left ">';
                		if(d.subs && d.subs.length){
                			tools_html += '<'+d.tagName+' href="'+d.href+'" id="' + create_id(d.id, container_id) + '" class="btn btn-default dropdown-toggle"  data-toggle="dropdown"  aria-expanded="true" title="' + d.title + '"><span class="' + d.iconClass + '"></span>' + (d.label||'');
                			tools_html += '<span class="caret"></span></'+d.tagName+'><ul class="dropdown-menu dropdown-menu-left">';
                			$.each(d.subs,function(i,d){
                				tools_html += '<li><'+d.tagName+' href="'+d.href+'" id="' + create_id(d.id, container_id) + '"  title="' + d.title + '"><span class="' + d.iconClass + '"></span>' + (d.label||'');
                				tools_html += '</'+d.tagName+'></li>';
                			});
                			tools_html += '</ul>';
                		}else{
                			tools_html += '<'+d.tagName+' href="'+d.href+'" id="' + create_id(d.id, container_id) + '" class="btn btn-default" title="' + d.title + '"><span class="' + d.iconClass + '"></span>' + (d.label||'');
                			tools_html += '</'+d.tagName+'>';
                		}
                		tools_html += '</div>';
            ...
            var res = $.ajax({
                ...
                success: function(data) {
                    ...
                    for(row in page_data) {
                        ...
                        for(i in s.columns) {
                            if(column_is_visible(s.columns[i])) {
                            	//if(s.columns[i].template){
                        		if (typeof (s.columns[i].template) === "function" ) {
                        			var cell_html = s.columns[i].template.call(elem, page_data[row][s.columns[i].field], page_data[row])
                        			tbl_html += '<td>' + cell_html + '</td>';
                        		} else if (typeof (s.columns[i].template) === "string" ) {
                        			tbl_html += '<td>' + s.columns[i].template.replace(/{value}/g,page_data[row][s.columns[i].field]) + '</td>';
                        		} else {
